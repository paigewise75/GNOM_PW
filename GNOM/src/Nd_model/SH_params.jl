...
p = Params(
    ...
    α_GRL = 4.07864, # 1.57
    σ_ε = 0.399108per10000, # 0.379per10000
    ...
    ϕ_∞ = 4.15497pmol/cm^2/yr, # 1.11pmol/cm^2/yr
    z_0 = 5002.15m, # 170.0m
    ε_EAsia_dust = -11.0691per10000, # -7.6per10000
    ε_NEAf_dust = -12.7453per10000, # -13.7per10000
    ε_NWAf_dust = -11.959per10000, # -12.3per10000
    ε_NAm_dust = -4.20368per10000, # -4.25per10000
    ε_SAf_dust = -18.9984per10000, # -21.6per10000
    ε_SAm_dust = -5.73316per10000, # -3.15per10000
    ε_MECA_dust = -1.94954per10000, # 0.119per10000
    ε_Aus_dust = -2.14714per10000, # -4.03per10000
    ε_Sahel_dust = -9.65416per10000, # -11.9per10000
    β_EAsia_dust = 3.48845per100, # 23.0per100
    β_NEAf_dust = 0.439373per100, # 23.3per100
    β_NWAf_dust = 4.42816per100, # 3.17per100
    β_NAm_dust = 3.58562per100, # 82.8per100
    β_SAf_dust = 3.84753per100, # 38.5per100
    β_SAm_dust = 1.76451per100, # 2.52per100
    β_MECA_dust = 0.206554per100, # 14.7per100
    β_Aus_dust = 3.57372per100, # 11.6per100
    β_Sahel_dust = 0.948513per100, # 2.95per100
    ε_volc = 6.93514per10000, # 13.1per10000
    β_volc = 4.42266per100, # 76.0per100
    K_prec = 0.072623/(mol/m^3), # 0.00576/(mol/m^3)
    f_prec = 0.901834, # 0.124
    w₀_prec = 0.7km/yr, # 0.7km/yr
    K_POC = 0.189972/(mol/m^3), # 0.524/(mol/m^3)
    f_POC = 0.77298, # 0.312
    w₀_POC = 40.0m/d, # 40.0m/d
    K_bSi = 76.5762/(mol/m^3), # 2.56/(mol/m^3)
    f_bSi = 0.603138, # 0.784
    w₀_bSi = 714.069m/d, # 714.0m/d
    K_dust = 1.17761/(g/m^3), # 1.7/(g/m^3)
    f_dust = 0.457392, # 0.0861
